...
---`,
    ...
    lodash_1.default.map(cheerioHighlightBody('.views-row .node'), function mapTheNodes(node) {
        ...
        const date = new Date(cheerioHighlightBody(node)
            .find('.timestamp')
            .text()
            .replace(/\s\(.*\)/gi, ''));
    ...
    lodash_1.default.forEach(highlightArray, function forEachHighlight(highlight) {
    ...
    for (var i = 0; i < highlightArray.length; i++) {
        let videoHtml = options.iframeTemplate.replace('{replaceMe}', videos[i]('video').attr('data-video-id'));
    ...
    const newPosts = await SendNewFilesToGitHubRepo(options, context, highlightArray);
    cb(null, { newPosts });
}
async function SendNewFilesToGitHubRepo(options, context, allHighlights) {
    ...
    let newPosts = lodash_1.default.differenceWith(allHighlights, previousUnitedPosts, function checkPreviousPostVsNew(mnufcValue, githubObject) {
...
${options.postHeader}
${post.video}`;
        const newBranchName = `refs/heads/${lodash_1.default.snakeCase(post.title)}`;
